...
class DepositJournalEntryWizard(models.TransientModel):
    ...
    booking_id = fields.Many2one('hotel.reservation',"Booking Ref",readonly=True,default=lambda self: self._get_default_rec())
        ...
        for obj in self:
            ...
            for reservation in self.env['hotel.reservation'].browse(self._context['active_ids']):
                ...
                if folio_obj:
                    ...
                    self._cr.execute('insert into sale_account_move_rel(sale_id,move_id) values (%s,%s)', (folio_browse[0].order_id.id, move_id))
