...
class CrmLead(models.Model):
    ...
    shop_id = fields.Many2one('sale.shop', 'Shop', required=True, help="Will show only open leads for the selected shop.")
                    ...
                    if not policy_browse[0].name == '24hour':
                        ...
                        for count in range(0, (int(day_count1))):
                            ...
                            if str(single_date)[0:10] == obj.checkin_date[0:10]:
                                ...
                                if obj.checkout_date > check_out_cons:
                                    wd_count += 1
                            if (str(single_date)[0:10] != obj.checkin_date[0:10]) and (str(single_date)[0:10] != obj.checkout_date[0:10]):
                                time_con = str(policy_browse[0].time)
...
#                'tour_name':fields.char('Banquet Name',size=64,readonly=True, states={'draft': [('readonly', False)]},),
    ...
    checkin_date = fields.Datetime("Prefer start Date", required=True, readonly=True, states={'draft': [('readonly', False)]})
    checkout_date = fields.Datetime("Prefer End Date", required=True, readonly=True, states={'draft': [('readonly', False)]},)
    ...
    number_of_days = fields.Integer(compute='count_total_days', string="Number Of Days",
                                    size=30, help="Shall be computed based on check out policy configured for selected shop.")
    ...
    deposit_policy = fields.Selection([('percentage', 'Deposit Percentage'), ('no_deposit', 'No Deposit')],
                                      'Deposit Policy', required=True, readonly=True, states={'draft': [('readonly', False)]}, default='no_deposit')
    ...
    shop_id = fields.Many2one('sale.shop', 'Shop', required=True, readonly=True, states={'draft': [(
        'readonly', False)]}, default=_get_default_shop, help="Will show list of shop that belongs to allowed companies of logged-in user.")
    company_id = fields.Many2one(related='shop_id.company_id', relation='res.company', string='Company', store=True)
...
#             raise exceptions.except_orm('Warning', 'Shop is not selected.')
        ...
        if self.checkin_date and self.shop_id.id:
            ...
            self._cr.execute('select max(start_date) from deposit_payment_policy where start_date <= %s and shop_id = %s', (new_date, self.shop_id.id,))
            a = self._cr.fetchone()
            if not a[0]:
                            ...
                            if (history_start_date <= reservation_start_date < history_end_date) or (history_start_date < reservation_end_date <= history_end_date) or ((reservation_start_date < history_start_date) and (reservation_end_date > history_end_date)):
                                if not (obj == history.booking_id):
                                    raise exceptions.except_orm(_("Warning"), _(
                                        "You tried to confirm reservation with room  %s which is already reserved in this reservation period !") % (room.room_number.name))
        ...
        for obj in self:
            ...
            if rejected_history_id:
                for id in rejected_history_id:
                    self.env['banquet.quotation.lead.history'].write({'state': 'refused', 'update_date': time.strftime('%Y-%m-%d')})
                      ...
                      #############################Added by Pornima############
                        ...
                        if housekeeping_room:
                            for house1 in housekeeping_room:
                                ...
                                if (house_current_date <= start_reser <= house_end_date) or (house_current_date <= end_reser <= house_end_date) or ((start_reser < house_current_date) and (end_reser > house_end_date)):
                                    ...
                                    # (house.state == 'dirty'):
                                    raise exceptions.except_orm(_("Warning"), _("Room  %s is not clean for reservation period !") % (room_line_browse[0].name))
                ...
                for rental_line in obj.reservation_line:
                    ...
                    if room_line_id:
                        ...
                        if room_line_id.deposit_bool:
                            ...
                            total += (rental_line.sub_total1 + self._amount_line_tax(rental_line)) * obj.percentage / 100
...
#             })
    ...
    deposit_recv_acc = fields.Many2one('account.account', string="Deposit Account",  required=False, company_dependent=True)
...
class FoodItems(models.Model):
    ...
    def _amount_line_tax_for_food(self, line):
        val = 0.0
        taxes = line.tax_id.compute_all(line.price_unit * (1 - (line.discount or 0.0) / 100.0), None, line.product_uom_qty)
        val = taxes['total_included'] - taxes['total_excluded']
        return val
...
#             })
    ...
    product_id = fields.Many2one('product.product', 'Product', domain=[(
        'sale_ok', '=', True)], change_default=True, help="Will list out all food items that belong to company of selected shop. \n It also shows global product as well.")
