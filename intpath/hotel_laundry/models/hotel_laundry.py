...
#     name = fields.Char('Name', size=30)
    partner_id = fields.Many2one('res.partner', 'Supplier Name', size=30, required=True, states={
...
class hotel_laundry_service(models.Model):
    ...
    category_id = fields.Integer('Category', size=20, default=get_category_id)
...
class hotel_laundry_service_items(models.Model):
    ...
    item_id = fields.Many2one(
        'product.product', 'Items', size=30, required=True)
    cost_price = fields.Float('Cost Price', size=30)
    sale_price = fields.Float('Sale Price', size=30)
    category_id1 = fields.Integer('Category', size=20, default=get_category_id)
...
class laundry_management(models.Model):
    ...
    deadline_date = fields.Datetime('Request Deadline', states={'confirmed': [('readonly', True)], 'laundry_returned': [(
        'readonly', True)], 'sent_to_laundry': [('readonly', True)], 'customer_returned': [('readonly', True)], 'done': [('readonly', True)]}, size=30)
...
#        'warehouse_id': fields.many2one('stock.warehouse', 'Warehouse', states={'confirmed':[('readonly',True)], 'laundry_returned':[('readonly',True)],'sent_to_laundry':[('readonly',True)],'customer_returned':[('readonly',True)],'done':[('readonly',True)]}, select=True),
    ...
    request_type = fields.Selection([('internal', 'Internal'), ('from_room', 'From Room')], 'Request Type', states={'confirmed': [('readonly', True)], 'laundry_returned': [
        ('readonly', True)], 'sent_to_laundry': [('readonly', True)], 'customer_returned': [('readonly', True)], 'done': [('readonly', True)]}, size=30, required=True,)
    room_number = fields.Many2one('hotel.room', 'Room No', states={'confirmed': [('readonly', True)], 'laundry_returned': [('readonly', True)], 'sent_to_laundry': [(
        'readonly', True)], 'customer_returned': [('readonly', True)], 'done': [('readonly', True)]}, size=64, help="Will show list of currently occupied room no that belongs to selected shop.")
...
class laundry_service_product_line(models.Model):
    ...
    qty_uom = fields.Many2one('uom.uom', 'UOM', size=30, default=_get_uom_id)
    qty = fields.Float('Quantity', size=30, default=1)
