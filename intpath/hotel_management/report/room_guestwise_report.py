...
class room_guest(models.AbstractModel):
    ...
    _description = 'roomwise guestwise report view'

    def get_roomtype_guest_information(self,data):
        print("object=========get_transport_information===",data)
        start_date = data.date_start
        end_date = data.date_end
        history_search = self.env['hotel.room.booking.history'].search([('check_in_date', '>=', start_date),('check_out_date', '<=', end_date),('booking_id.company_id', '=', self.env.user.company_id.id)])
        final_res = {}
        result_acc = []
        cus_res = {}
        key_list = []
        ...
        for key in key_list:
            ...
            for room in history_search:
                if room.history_id.name == key:
                    ...
                    if folio_id:
                        is_checkin = "Yes"
                    folio_out_id  = self.env['hotel.folio'].search([('reservation_id', '=', room.booking_id.id),('state', 'in', ['check_out','done'])])
                    if folio_out_id:
                        is_checkout = 'Yes'
                    cus_res = {
...
#         return self.env['report'].render('hotel_management.roomwise_guestwise_report_view', docargs)
