...
class monthly_occupency(models.AbstractModel):
    ...
    _description = 'monthly 1occupency report view'

    # @api.multi
    def get_monthly_occupancy_information(self, data):
        #         period_id = data['form']['period_id'][0]
        #         period_id = self.pool.get('account.period').browse(self.cr, self.uid, period_id)
        print(type(data.start_date),'starrrrrrrrrrrrrrrrrrrrrr')
        ...
        if hotel_search:
            ...
            for folio in hotel_search:
                print(folio.reservation_line, '===========res line')
                for line in folio.reservation_line:
                    if line.checkin and line.checkout:
                        ...
                        print("earlyaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",no_days,type(reservation_start_date),type(history_start_date1),type(reservation_end_date))
        ...
        print("folio_search  ",folio_search)
        if folio_search:
            for folio in folio_search:
                if folio.room_lines:
                    for date in folio.room_lines:
                        #print("sdhfhsdjfhsdhfjsdfdfsdfgdsfhdgfhsdgfhsdgfgdfsd", folio.rooom_lines)
                        history_start_date = date.checkin_date
                        history_end_date = date.checkout_date
                        print("11111111111111111111111111111111111111111",type(history_start_date),type(reservation_start_date),type(history_end_date),type(reservation_end_date),)
            ...
            if not updated_folio:
                for count in range(0, (int(day_count) + 1)):
                    result_dic = {}
                    print("\n\n\n\n\nReservation Start Date  ===   ",reservation_start_date)
                    print("Count ========   ",count)
                ...
                print(updated_folio, "updated_folio")
                total_room = len(self.env['hotel.room'].search([]))
                for folio in updated_folio:
                    for count in range(0, (int(day_count) + 1)):
                        result_dic = {}
                        print("\n\n\n\n\nRiservation Start Date  $$$$$$  ",reservation_start_date)
                        print("Count $$$$$$   ",count)
                        single_date = reservation_start_date + timedelta(days=count)
                        print("SIngle Dateee  ",single_date)
                        if not single_date in date_list:
                            date = single_date

            #                start_date = single_date + ' 00:00:00'
                            end_date = date
                            print("222222222222222222222222222222222222222222222222",end_date,type(end_date))
                            #start_date = datetime.strptime(single_date, '%Y-%m-%d %H:%M:%S')
            ...
            #                trans_amt = 0
                            ...
                            for folio in updated_folio:
                                ...
                                if (history_start_date <= start_date < history_end_date) or (history_start_date < end_date <= history_end_date) or ((start_date < history_start_date) and (end_date >= history_end_date)):

                                    for now_days in folio.reservation_id.reservation_line:
                                        no_days= now_days.number_of_days
                                    #print(folio.reservation_id.reservation_line.number_of_days, "RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR")
                                    #no_days+=no_days
                                    ...
                                    for laundry_data in folio.laundry_line_ids:
                                        if str(laundry_data.source_origin) not in calculated_origins:
                                            laundry_data_search = self.env['laundry.management'].search([('name', '=', laundry_data.source_origin)])
                                    ...
                                    for table_data in folio.food_lines:
                                        table_search = self.env['hotel.restaurant.order'].search([('order_no', '=', table_data.source_origin)])
                                        order_search = self.env['hotel.reservation.order'].search([('order_number', '=', table_data.source_origin)])

                                        if table_search:
                                            table_id = table_search
                                            if table_search:
                                                if order_search.o_date == date:
...
#         return self.env['report'].render('hotel_management.monthly_occupency_report_view', docargs)
