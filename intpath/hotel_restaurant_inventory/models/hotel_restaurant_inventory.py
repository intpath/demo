        ...
        for wizard_line in self.move_ids:
            ...
            if line_uom.factor and line_uom.factor != 0:
                if float_compare(qty_in_line_uom, wizard_line.quantity, precision_rounding=line_uom.rounding) != 0:
                    raise UserError('The unit of measure rounding does not allow you to ship "%s %s", only rounding of "%s %s" is accepted by the Unit of Measure.') % (
            ...
            if move_id:
                ...
                if float_compare(qty_in_initial_uom, without_rounding_qty, precision_rounding=initial_uom.rounding) != 0:
                    raise UserError('The rounding of the initial uom does not allow you to ship "%s %s", as it would let a quantity of "%s %s" to ship and only rounding of "%s %s" is accepted by the uom.') % (
                        wizard_line.quantity, line_uom.name, wizard_line.move_id.product_qty - without_rounding_qty, initial_uom.name, initial_uom.rounding, initial_uom.name)
            else:
        ...
        for order_list in self_obj.kot_list:
            if order_list.state == "in_process":
                ...
                for b_id in bom_record:
                    ...
                    print("bom_browse.product_id.name == order_list.product_id.name--", bom_browse.product_id.name, order_list.product_id.name)
                    if bom_browse.product_id.name == order_list.product_id.name:
                        ...
                        context = {'lang': 'en_US', 'tz': False, 'uid': self._uid, 'active_model': 'mrp.production', 'active_ids': [
        ...
        for obj in self:
            ...
            if obj.order_list == []:
                ...
                for line in obj.order_list:
                    ...
                    print("menu_card_objjjjjjjjj", menu_card_obj, "------menu_card_obj.route_ids:----", menu_card_obj.route_ids)
                ...
                for order_items in self_obj.order_list:
                    ...
                    if move_id:
                        if not order_items.total:
                            ...
                            total_qty = 0
                            print("order_items.product_id.id", order_items.product_id.id, "self_obj.order_no", self_obj.order_no)
    ...
    def create_invoice(self):
        ...
        for self_obj in self:
            ...
            if stock_brw:
                ...
                print('\n\nsending contextttt========', context)
                res = self.env['stock.partial.picking'].with_context(context).default_get(['date', 'picking_id', 'move_ids'])
                print("resssssss", res)
...
#
    ...
    def done_kot(self):
        ...
        for b_id in bom_record:
            ...
            print("bom_browse.product_id.name == self.product_id.name", bom_browse.product_id.name, self.product_id.name)
            if bom_browse.product_id.name == self.product_id.name:
                ...
                bom_obj.action_ready()
                bom_obj.action_in_production()
                context = {'lang': 'en_US', 'tz': False, 'uid': self.env.uid, 'active_model': 'mrp.production', 'active_ids': [
                    b_id], 'search_default_ready': 1, 'active_id': b_id}
