...
#         self.localcontext.update(
    ...
    def _get_payments(self, form):
        print("dayanand in credit sales-------------------------")
        statement_line_obj = self.pool.get("account.bank.statement.line")
        pos_order_obj = self.pool.get("pos.order")
        print("pos_order_obj ------------",pos_order_obj) 
        for pos_order_obj1  in pos_order_obj:
            print(pos_order_obj1.state)
        user_ids = form['user_ids'] or self._get_all_users()
...
#         super(pos_credit_sale, self).__init__(cr, uid, name, context=context)
    
#         )

# report_sxw.report_sxw('report.pos.credit.details', 'pos.order', 'addons/hotel_restaurant_pos/report/pos_credit_sale.rml', parser=pos_credit_sale, header='internal')
