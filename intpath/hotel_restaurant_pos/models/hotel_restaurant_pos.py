...
#   END    ========= COMMENTED BY RADHIKA ============= 
    ...
    table_ids = fields.Many2many('hotel.restaurant.tables','pos_book_tables','table_no','name','Table number',size=64)
    ...
    credit_sales = fields.Char('creditsales', size=32)
    waiter_name = fields.Char('Waiter Name', size=32)
    ...
    def action_paid(self):
        ...
        for order in self.env['pos.order'].browse():
            ...
            if order.credit_sales=='True':
                ...
                print("in create picking------------ttttttttttttttt")
                self.create_picking()
                self.write({'state': 'paid'})
...
class HotelReservationOrder(models.Model):
    ...
    pos_ref = fields.Char('POS Ref.',size=128)
...
class PosConfig(models.Model):
    ...
    def open_ui(self):
        if not self.shop_id:
            raise UserError("Shop must be define before starting your shopping. (Under Shop Setting--> Click on checkbox Barcode Scanner -->Shop Name)")
    ...
    def _open_session(self, session_id):
        if not self.shop_id:
            raise UserError("Shop must be define before starting your shopping. (Under Setting--> Click on Checkbox Barcode Scanner -->Shop Name)")
        else :
